...
// ==========================================
// Guarda el pick propio del Draft en Firestore, bajo el mismo documento de
// "partidas_en_vivo" que usa el resto de la carrera compartida, pero en un
// campo separado (draft_slotA / draft_slotB) para no pisar el resumen de
...
async function guardarPickPropioCompartido(datosPick) {
    ...
    try {
        ...
        const campo = `draft_${gameState.player.slotPropio}`; // draft_slotA o draft_slotB
...
// Busca el pick del rival compartido (si ya hizo su Draft).
...
// 🆕 Polling: reintenta cada 5s buscar el pick del rival mientras estés
// parado en la pantalla del Draft, por si drafteó después que vos. Se
// detiene solo (clearInterval) apenas lo encuentra o cuando avanzás de
// pantalla (ver iniciarTemporadaUno).
let intervaloDraftRival = null;

function iniciarPollingRivalDraft(pick, equipo) {
    if (!tieneCarreraCompartida()) return;
    detenerPollingRivalDraft(); // por si quedó uno viejo corriendo
    ...
    intervaloDraftRival = setInterval(async () => {
        ...
        detenerPollingRivalDraft(); // ya lo encontramos, no hace falta seguir preguntando
    }, 5000);
}
